...
- 数据备份：支持导出/导入 JSON 文件，也支持备份到 GitHub Gist 并从 Gist 一键导入

AI 配置和解析缓存只保存在浏览器本地，不会写入仓库。默认接口地址是 `https://gcli.ggchan.dev`。

备份内容包含错题、AI 配置（不含 API key）和 AI 解析缓存。GitHub Gist 同步需要填写具备 `gist` 权限的 GitHub token；该 token 只保存在当前浏览器本地，不会进入备份文件。
